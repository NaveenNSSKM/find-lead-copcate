...
// ─── Type matches `findlead-ai-blog` table ────────────────────────────────────
interface BlogPost {
    id: number;
    title: string;
    category: string;
...
function SkeletonCard() {
    return (
        <div className="bg-white rounded-[40px] overflow-hidden shadow-soft flex flex-col h-full animate-pulse">
            <div className="h-64 bg-gray-200" />
            <div className="p-8 flex flex-col flex-grow gap-4">
                <div className="h-5 bg-gray-200 rounded w-3/4" />
                <div className="h-4 bg-gray-100 rounded w-full" />
                <div className="h-4 bg-gray-100 rounded w-5/6" />
                <div className="h-10 w-10 bg-yellow-100 rounded-full mt-auto" />
            </div>
        </div>
    );
}
...
export default function BlogPage() {
    ...
    useEffect(() => {
        async function fetchPosts() {
            ...
            const { data, error } = await supabase
                ...
                .select('id, title, category, description, content, image, status')
                .eq('status', 'published')
                .order('id', { ascending: false });

            if (error) console.error('[Blog] Supabase error:', error.message);
            setPosts(data ?? []);
            setLoading(false);
        }
        fetchPosts();
    ...
    // ── Scroll to top when post is opened ─────────────────────────────────────
    useEffect(() => {
        ...
        <main className="min-h-screen bg-[#EFE34B]">
            ...
            <div className="pt-32 pb-16 px-6 md:px-12 max-w-[1440px] mx-auto">
                ...
                {!loading && posts.length === 0 && (
                    <p className="text-[#121212] text-lg font-medium opacity-60 mt-8">
                ...
                {!loading && featuredPost && (
                    <div
                        className="bg-white rounded-[40px] overflow-hidden shadow-soft mb-12 flex flex-col lg:flex-row hover:shadow-xl transition-shadow cursor-pointer"
                    ...
                    >
                        ...
                        <div className="lg:w-1/2 h-[400px] lg:h-auto relative bg-gray-100">
                            {featuredPost.image && (
                                <Image
